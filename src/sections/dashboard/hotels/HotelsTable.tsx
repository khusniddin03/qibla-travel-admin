...
import { deleteHotel } from "./hotels.api";
...
interface IProps {
    ...
    error: any;
    count: number;
    rowClick: (data: IHotel) => void;
    setEditItemValue: (value: IHotel | null) => void;
    editItemValue: IHotel | null;
...
const HotelsTable = ({
    ...
    const columns = {
        ...
        description: {
            path: "description",
            name: "Mehmonxona haqida ma'lumot",
            component: (rowData: IHotel) => (
                <Typography
                    variant="body2"
                    fontSize={16}
                >
                    {rowData?.description?.uz ?? ''}
                </Typography>
            )
        },
        edit: {
    ...
    const { mutateAsync, isLoading: deleteIsLoading } = useMutation(deleteHotel);
